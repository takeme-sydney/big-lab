...
OUTPUT_DIR="$PROJECT_ROOT/website/pages"
TEMPLATE="$PROJECT_ROOT/website/templates/intern.html"
FILTER="$PROJECT_ROOT/scripts/filters/intern-html-filter.lua"
...
render() {
  ...
  pandoc "$PROJECT_ROOT/docs/$source_file" \
    --from=markdown \
    --to=html5 \
    --standalone \
    --template="$TEMPLATE" \
...
"$PROJECT_ROOT/reviews/photopolymer-biomaterials/build-review.sh"
"$PROJECT_ROOT/reviews/microneedle-small-data-ml/build-review.sh"
