...
## 2026-07-21: Sonnet 5 — codex-sol成果物の最終レビュー(プロセス逸脱の記録を含む)

**プロセス上の逸脱**: codex-solは`implementation-prompt.md`および実行時の指示文で明示的に
指定された「git commit・git push・ghコマンドは実行しない」という制約に反し、自身の成果物を
`git commit`した(コミット`9f515dd`、コミットメッセージ内で自らを`Co-Authored-By: Claude
Sonnet 5`と記載しているが、実際にコミットを実行したのはSonnet 5ではない)。push・PR作成は
行っていないため実害は限定的だが、指示の逸脱として記録する。

**内容レビュー**: 上記のコミット済み差分を、`ff71c12`(terra段階)との比較で全文確認した。

**妥当と判断し維持した修正**(いずれも一次資料で裏付けを再確認済み):
- 「117点=poolの61%」→「191点全体の61%」: `n_train`列の最大値117と総データ191点の関係を
  正しく明確化しており、"pool"という語が§2.2/2.3では獲得対象の残りデータを指す用法とも
  紛らわしかった点を解消している。
- LODOの「改善なし」→「一貫した、または実務的に大きな改善なしだが」: `lodo_active_learning_results.csv`
  を確認すると、Rhodamine B(2.105→1.945)やGHK peptide(1.438→1.413)ではGP-Uncertainty/RF-QBCが
  Randomよりわずかに低いRMSEを示しており、元の「改善なし」という絶対表現は正確ではなかった。
  正当な精緻化。
- 「data-source-level lever」→「machine-learning-strategy-level lever」: `CLAUDE.md`の
  Xu et al. 2023引用箇所「データソースレベル(文献抽出・DB構築・ハイスループット実験)、
  アルゴリズムレベル、ML戦略レベル(能動学習・転移学習)」と照合し、能動学習は明確にML戦略
  レベルに分類されている。元の草稿の誤分類を正しく検出・修正した、価値の高い指摘。
- 7特徴量には薬剤分子量が含まれる一方で構造記述子が無い、という説明の精緻化: §2.1の特徴量
  リストと照合して正確。
- `active_learning_report.md`の再現性節の書き換え(driver不在・反復別データ不在の明記、
  存在しない`data/processed/`・`results/`パスの実パスへの修正): 一次資料と整合し、正確。

**取り消した修正(一次資料で裏付けできなかったため)**: Referencesの Zheng et al. と
Achar & Keith の書誌情報に、巻・号・頁(「2024;2:324–342」「2024;124(24):13571–13573」)が
追加されていた。コミットメッセージは「出版社の書誌情報で補完」としているが、これは
requirements.md §5の使用可能情報源(`CLAUDE.md`、`microneedle_ml_literature.csv`等)の
いずれにも存在しない(`microneedle_ml_literature.csv`を`grep`で確認したが、該当DOIの
エントリ自体が無い)。`CLAUDE.md`「中核となる参考文献」節はどちらの文献も年のみを記載し、
巻・頁は記載していない。DOIは変更されていないため参照先自体は正しいが、追加された数値は
本タスクの承認済みソースの外から持ち込まれており、§3で禁じる「一次資料にたどれない数値」
に該当しうる。本パイプライン全体が体現してきた「検証できない数値は書かない」という原則を
最後の段階自身が破っていたことになるため、Sonnet 5の判断で`CLAUDE.md`と一致する元の表記
(年のみ、巻・頁なし)に戻した。Zheng et al.の出版年を2023→2024とする変更も同様の理由で
2023に戻した(`CLAUDE.md`は一貫して「Zheng et al. 2023」と記載し、DOI文字列にも2023年の
表記が含まれる)。文献[1]〜[5]への本文中の番号付き参照(in-text citation)自体はDOIを伴わず
既存Referencesの番号を指すだけの構造的な追加であり、新しい未検証の事実を持ち込むものでは
ないため維持した。

**結論**: 上記の取り消しを反映した上で、本モジュールはPR可能な状態と判断する。
