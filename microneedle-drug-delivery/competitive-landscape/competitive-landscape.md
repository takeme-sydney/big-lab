...
*2026-07-20追記: Fable-5が利用制限で2回失敗したため、Claude Sonnet 5がこのセッション内で
直接フォローアップ調査を実施した(WebSearch/WebFetch使用)。結果を以下に統合する。*

1. **美容成分への応用の独自性は、依然として明確な答えが得られていない。** 追加調査で
   確認できたのは: (a) 一般的な皮膚透過性QSAR/ML研究は「医薬品・化粧品双方への応用」を
   目的として繰り返し言及している(例: HSVR+PLSの"Two-QSAR"手法、PMC9143389/MDPI
   14(5):961)— つまり化粧品はこの分野で頻繁に引用される応用先ではあるが、
   **本プロジェクトと同じ枠組み(microneedle特化 + transfer learning + 特定48成分リスト)
   に一致する先行研究は見つからなかった**。(b) "microneedle cosmetic ingredient machine
   learning permeation prediction" で直接検索しても、Yuan et al. (2023) 自身とBiswas et al.
   (2024, 未adversarial verification、§9参照)以外に該当する研究は見つからなかった。
   (c) 「化粧品成分の皮膚感作性(skin sensitization)予測のML研究」(PMC11648681)も
   見つかったが、これは透過性ではなく安全性エンドポイントであり、本プロジェクトとは
   別の問題設定なので混同しないこと。**結論: 「microneedle × 化粧品成分 × 構造ベースQSAR」
   という組み合わせについては、引き続き未確認(先行研究なしとは断定できないが、
   今回の調査では見つからなかった)。**
2. Asgarkhanova et al.の実際の手法・性能指標は**依然として不明**。追加調査でWiley本体の
   抄録ページ(https://onlinelibrary.wiley.com/doi/abs/10.1002/minf.70030)から
   「209件のキュレーション済み化合物、Kp値とメタデータを含む」という抄録レベルの記述を
   確認できたが(既知の情報と整合)、具体的な手法・R²/RMSE等の性能指標は本文なしには
   得られなかった。Varnek氏・Horvath氏(共著者)のResearchGateプロフィールは見つかったが、
   論文自体のプレプリントやPDFへのリンクは見つからなかった。**本文入手は引き続き未達成。**
3. HuskinDBの現行フル公開データが129化合物より多い可能性について、`research/`配下の
   全ローカルファイル(CSV・Markdown)を検索したが、251化合物や546測定値、
   コンセンサス値抽出のロジックについての説明・言及は**見つからなかった**。この差分の
   説明はローカル資料からは得られず、HuskinDBの現行公開データを直接確認する以外に
   解決方法がない。
4. 高分子量・biologics領域(microneedleが受動透過に対して優位性を発揮しやすい化学空間)を
   競合ツールがカバーしているかどうかは、今回のフォローアップ調査では扱っていない
   (優先度が最も低いため)。引き続き未確認。

## 9. 直接確認したリード

- **小林製薬**(日本企業)の技術ページ(https://skincare.kobayashi.co.jp/field/skincare/penetration02.html)を**直接fetchして内容を確認した**(2026-07-20)。確認できた内容:
  - 手法: ラマン分光スペクトル + 機械学習(MCR-ALS、多変量スペクトル分解)による、3次元培養皮膚モデル内部への美容成分の浸透可視化。
  - 対象成分: **トラネキサム酸**(美白・抗炎症成分。本プロジェクトの48件リストと同カテゴリ)。
  - 手順: (1) 3D皮膚モデルと純粋成分のRaman基準スペクトル取得 → (2) 多変量スペクトル分解による対象成分シグナルの分離 → (3) 実際の複合処方(クリーム等)での検証 → (4) MLによる皮膚断面への浸透分布の可視化。
  - IFSCC Congress 2025でポスター発表済み。
  - **本プロジェクトとの違い**: 手法が構造記述子からの透過係数(log Kp)予測ではなく、実測スペクトルの信号分離・可視化である。問題設定(ML×皮膚透過×美容成分)は共通するが、アプローチは全く異なる。本プロジェクトの構造ベースQSAR/転移学習アプローチと直接競合するものではないが、「日本企業がこの交差領域に既に取り組んでいる」という文脈として言及する価値がある。

## 10. 推奨アクション(優先順位順)

1. **[最優先]** Asgarkhanova et al. (2026) の本文入手(図書館アクセス等)を試み、本プロジェクトの214化合物データセットとの関係(再利用か独自統合か)・実際の手法/性能を明確化する。入手できるまでは、本プロジェクトの成果物で「データセットを構築した」ではなく「Asgarkhanova et al.が構築したデータセットを再利用している」という表現に統一する。
2. 美容成分への応用の独自性について、§8-1で示した通り一般的なQSAR研究は化粧品応用に言及するが、microneedle特化の先行研究は見つかっていない — この「見つからなかった」こと自体を、本プロジェクトの成果物で（「独自」と断定せず）正直に記述する。
3. Stevens et al. (2024) の層別diffusion/partitionデータをRESEARCH_PLAN.mdフェーズ4Bの物理モデル併用データ拡張の候補データ源として追加する(実施済み — RESEARCH_PLAN.md参照)。
4. Abdallah et al. (2024) との方法論比較(leave-one-drug-out vs ランダム分割)を、本プロジェクトの方法論的な強みとして明記する(実施済み — RESEARCH_PLAN.mdフェーズ6参照)。
5. MAMLスタイルのメタ学習を、単純なpretrain→fine-tuneの代替候補として弱く言及する(確信度: 中、皮膚特化の実証なし。実施済み — RESEARCH_PLAN.mdフェーズ4B参照)。
6. 小林製薬の技術(§9)を、美容成分への応用の独自性の議論における「関連するが異なるアプローチの例」として、将来の成果物のRelated Workで言及する。
